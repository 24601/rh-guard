...
| DSH (generic/adapter) | generic stdin (`hooks/run.ts dsh` or `generic`); Claude/Codex command-hook bridges also work | `hooks/run.ts dsh` or `generic` | HTTP skipped (404); generic `{block,reason}` + exit 2 |
