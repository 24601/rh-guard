...
| DSH (generic/adapter) | Claude/Codex command-hook bridges | `hooks/run.ts claude` or `codex`; also `dsh`/`generic` stdin | HTTP skipped; command deny / generic `{block}` + exit 2 |
