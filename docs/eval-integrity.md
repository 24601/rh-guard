...
[actiongate-jev](https://github.com/omkarghugarkar007/actiongate-jev) is runtime authorization for AI-agent tool calls before side effects. Deterministic policy + RBAC + schemas + amount/limit checks own `ALLOW` / `REVIEW` / `BLOCK`; TypeSafe Jev via OpenRouter supplies six narrow semantic signals (alignment, target match, policy conflict, sensitive-data exposure, scope expansion, missing intent) — never one vague "is this safe?" question, never generated prose as an authorization reason. **Jev supplies evidence. Code owns authority.** A positive model score never overrides a deterministic security failure (RBAC, schema, or limit). **Schema-valid ≠ intent-matched.** Early public MVP: use mock or sandbox tools; ActionGate decides and the application owns execution and credentials. Explicit [threat model](https://github.com/omkarghugarkar007/actiongate-jev/blob/main/docs/threat-model.md): the current SDK integration is advisory until an execution proxy; agents cannot self-declare a safer risk class; financial / destructive / credential actions fail closed when Jev is unavailable. The 500-case default evaluation is a label-baseline integrity run, not a claim of model accuracy — measure false-allow and false-block separately; do not treat Jev ALLOW as a safety proof. Compose with [construct-auto-classifier](https://github.com/godspede/construct-auto-classifier) (effect-based shell) and [jev-lens](https://github.com/rashedInt32/jev-lens) (attention≠authority, never blocks): ActionGate is the hard-gate + soft-evidence pattern for tool side-effects. Anti-pattern: hard-gating soft judgment as the sole safety control (soundness theater). Cousin, not this sidecar. Ranking ≠ calibration, so threshold-as-safety is fragile — [jevcal](https://github.com/Adilmp/jevcal) (Augustus owns the full mental model).
...
[jev-intent-review](https://github.com/yottayoshida/jev-intent-review) is under construction (spec only, 2026-09-19): typed Jev judgments of the post-change repository against originating intent. The diff is a search hint, not the object being verified (`review_scope != changed_files`). Jev judgment → deterministic policy → finding; thresholds live in code. Prefer UNKNOWN over a false VERIFIED. Never claims that finding nothing means the code is correct. Watch, not an endorsement. Cousin of jev-reviewer / ci-gatekeeper.
