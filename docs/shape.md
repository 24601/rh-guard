...
B is the base. The TypeSafe guardrails cookbook and the Pi `pi-jev-auto-mode` gate both keep rules first. Graft from A is the question catalog and the 2s timeout. Same layering, other products: [jevgate](https://github.com/thevibeworks/jevgate) proves allowlisted commands in code, then asks Jev only about the rest. [JevLint](https://github.com/huntedman/JevLint) uses Jev for convention Nouls in a write → check → fix loop (quality, not eval-gaming). Do not merge them into this sidecar.
