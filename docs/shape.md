...
B is the base. The TypeSafe guardrails cookbook and the Pi `pi-jev-auto-mode` gate both keep rules first. Graft from A is the question catalog and the 2s timeout. [jevgate](https://github.com/thevibeworks/jevgate) is the same layering on shell permissions: an allowlist proves what may run, code-refused writers never reach the model, Jev judges only unlisted verbs, and the tool cannot block. [JevLint](https://github.com/huntedman/JevLint) uses Jev for convention Nouls in a write → check → fix loop (quality, not eval-gaming). Do not merge them into this sidecar.
