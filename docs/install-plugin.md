...
| dsh | DeepSeek Harness Claude/Codex **command** bridges (`hooks/run.ts claude` or `codex`). HTTP skipped. Generic stdin: `hooks/run.ts dsh` / `generic` with [`examples/generic-event.json`](../examples/generic-event.json). |
