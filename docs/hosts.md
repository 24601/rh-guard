...
Host-adapter cousin (not this copy): [jev-routing](https://github.com/nekowasabi/jev-routing) — Go binary harness for Cursor Agent CLI among others; explicitly **not an MCP server**; drop/truncate **without summarizing**; default `filter`; `forced` only with a verified real Jev answer; `tools[]` → 1 schema.
...
Host-adapter cousin (not this copy): [jev-routing](https://github.com/nekowasabi/jev-routing) — Go binary harness; explicitly **not an MCP server**; drop/truncate **without summarizing**; default `filter`; `forced` only with a verified real Jev answer; PreCompact cannot strip the catalog.
