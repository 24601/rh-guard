...
[apixly-ai/jev-filter](https://github.com/apixly-ai/jev-filter) (MIT, Python, HEAD `0d62a90`; current listing of [apixly-ai/jev-context](https://github.com/apixly-ai/jev-context), which redirects here) is an evidence CLI. README title remains **jev-context**. The CI badge and install URL still point at `JIA-ss/jev-context` (`git+https://github.com/JIA-ss/jev-context.git@v0.1.0`), not this listing. **advertised install remote ≠ this listing.** GitHub description (*theirs*): evidence-preserving semantic CLI. Quoted: **A semantic judgment never grants permission to execute an action.** Quoted: **`exec` executes the command you supply; it is not a sandbox.** `doctor` and `--plan` work without a key. No result cache. Up to **30** concurrent requests. Context reduction is not end-to-end cost reduction. Independent, maintained by JIA-ss, not an official TypeSafe product. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
