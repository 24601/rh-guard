...
[bismawy/pi-jev-eye](https://github.com/bismawy/pi-jev-eye) (GitHub license field null; README badge **MIT**, TypeScript, HEAD `7cedcce`) is a Pi supervisor. Layer 1 regex blocks `rm -rf /`, force-push, and secrets at 0 ms. Layer 2 is a done-check warning notice if the agent claims done without tests. Layer 3 asks Jev `has_slop` on diffs of at least 10 lines; **P ≥ 0.85 blocked**. Quoted **Fail-Open Safety:** **Network timeouts or offline Jev API calls gracefully fall back to allow work to continue without freezing the agent.** `/eye off` disables the supervisor. **0.85** is uncalibrated and is not a rh-guard ROC. The regex floor is the hard envelope; Jev fail-open is not a hard deny. Do not merge into `examples/pi-extension.ts`. Distinct from [DevMortimer/pi-warden](https://github.com/DevMortimer/pi-warden), [Reindeer-AI/pi-jev-guard](https://github.com/Reindeer-AI/pi-jev-guard), [phin-tech/pi-jev-approver](https://github.com/phin-tech/pi-jev-approver), and [fivethirty/pi-jev-gate](https://github.com/fivethirty/pi-jev-gate). Soft judgment is never the sole veto. Cousin, not this sidecar.
