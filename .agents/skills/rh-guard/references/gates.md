...
[construct-auto-classifier](https://github.com/godspede/construct-auto-classifier) is an effect-based OpenCode / Antigravity (`agy`) shell PreToolUse gate: structural fast-deny/fast-allow first, then Jev Choice plus **eleven** risk questions, one per harm (HEAD `531a39f`; the earlier gate’s nine Nouls included `data_loss`). Allow only if the choice is `allow`, every risk is below `jev.riskThreshold` (**0.7**), and confidence meets `jev.minConfidence` (**0.6**, which they define as p(allow) ≥ 0.8) unless every risk is below `jev.lowRiskCeiling` (**0.2**). Quoted README: **Fails closed.** Quoted: **Not a sandbox.** *Theirs* current bar: **0** dangerous allowed on main **181** plus blind **79** (blind **395/395**). The earlier chat-model table does not describe this code. [Qwen3.8-27B](https://huggingface.co/Qwen/Qwen3.8-27B) is not Archer. Archer stays **promised-not-landed**. Operator owns the dial. Privilege Is Not a Verdict. Pair with dinostomp before hard-gating on those scores. Do not merge into `examples/`. Cousin, not this sidecar.
...
[jevgate-action](https://github.com/ktsu2i/jevgate-action) (MIT, Shell, HEAD `41d5706`) composite `action.yml` and `action.sh` on `main`; `.github/workflows/ci.yml` and `release.yml`. Quoted README: **This Action has not been published yet.** **0 Releases.** Name collision with Tech-Byte-Frontier/jevgate-action. Distinct thevibeworks/jevgate and totally-tim/jev-gate. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
[herdr-jev-router](https://github.com/boriscardano/herdr-jev-router) (Apache-2.0, Python, HEAD `485e9c9`) advisory routing. Quoted: **It is advisory.** Quoted: **It is not enforcement.** **Treat the enforcement claims as unproven.** Fail closed. The caller still cannot choose one. Distinct muthuishere/herdr-jev. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
[WesleySmits/spark-jev-email-triage](https://github.com/WesleySmits/spark-jev-email-triage) HEAD `12b0744` reads Spark mail; classifications do not change mail; archive is default-off after explicit approval; the heading-only claim is stale. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
[itsmostafa/typesafe-mcp](https://github.com/itsmostafa/typesafe-mcp) one-tool `evaluate`; Laya via `TYPESAFE_BASE_URL`, weights not bundled; hope-the-model-looks. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
[AkashPriyadarshii/jev-seo](https://github.com/AkashPriyadarshii/jev-seo) Quoted **Thresholds live in one place.** Description rewrite (*theirs*): **53-rule** audits, **13-tool** MCP, spend hard-capped. Not a measured ROC. Distinct jev-git. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
[Tech-Byte-Frontier/jevgate](https://github.com/Tech-Byte-Frontier/jevgate) (README: MIT OR Apache-2.0, Rust, HEAD `dfe3be5`) description rewrite (*theirs*): Code-review gate for CI and coding agents. The prior advisory-only and not-yet-implemented enforcement quotes are absent at this HEAD. Quoted README: `fail_on = ["none"]` or `--fail-on none` reports findings without failing. Quoted: a finding that fails the gate is an error. Quoted: a run that could not finish (missing key, provider rejection, request budget) still exits 2, so an outage never passes as a clean review. Quoted: **Probabilities:** these are model judgments, not measured accuracy. Distinct from [thevibeworks/jevgate](https://github.com/thevibeworks/jevgate). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
...
[robinwintertaylor/Prompt-Router](https://github.com/robinwintertaylor/Prompt-Router) (LICENSE file present; badge MIT; TypeScript, HEAD `3117ae6`) description rewrite (*theirs*): Sub-120ms smart LLM gateway and a 0.60 confidence-gated safety. Prior fold said no license file and that badges claim **Zero Hallucination Routing**. At this HEAD the README quotes **We do not claim "zero hallucinations"**. A Zero-Key Heuristic Mode runs without a TypeSafe key. Quoted: below **0.60** confidence it refuses to downgrade and elevates to frontier models. *Theirs* ~**68%** on TypeSafe's four-workflow benchmark is not a rh-guard ROC. **0.60** is uncalibrated; elevation is not a deny. Routing ≠ permission. Distinct from [fstandhartinger/jev-router](https://github.com/fstandhartinger/jev-router) and [0xNatoshi/jev-codex-router](https://github.com/0xNatoshi/jev-codex-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
...
[HCTDIP/jeveto](https://github.com/HCTDIP/jeveto) (MIT, Python, HEAD `242395c`; renamed from [HCTDIP/mule-run](https://github.com/HCTDIP/mule-run) on 2026-09-25 because of a name collision) is a FastAPI orchestration MVP. Quoted README: confidence **≥ 0.85** executes; otherwise a saved handoff. The confidence formula is `p_i ∝ 1/(1+score)` in code, not a TypeSafe probability. Quoted DONE check is `completed == total`. *Theirs* **16/16** smoke tests are not a rh-guard ROC. **0.85** is uncalibrated. This listing is not a different mule-run product. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
...
[qinpei-dev/permit-mcp](https://github.com/qinpei-dev/permit-mcp) (GitHub license `NOASSERTION`; Python, HEAD `5268635`) is an MCP execution layer. Quoted README: **Policy `DENY` stops before JEV.** Policy `REVIEW` waits for caller approval. `ALLOW` receives a one-use permit. Quoted: these outcomes are an application-layer interpretation of TypeSafe Choice, **not a separate TypeSafe Gate primitive**. The showcase uses an offline JEV mock; `../secret.txt` never reaches JEV or the executor. Tools include `jev_decide`, `controlled_agent_run`, and `approve_action`. With no `JEV_API_KEY` the local mock runs. Distinct from [qinpei-dev/Doubao-jev-agent](https://github.com/qinpei-dev/Doubao-jev-agent). A mock choice is not a live verdict. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[akanksha-rajhans-ai/diffguard](https://github.com/akanksha-rajhans-ai/diffguard) (MIT, TypeScript, HEAD `44d28e2`) is pull-request risk triage. Quoted README: **DiffGuard prioritizes review attention. It does not certify that code is correct or replace human review.** Jev emits typed risk judgments; a deterministic policy engine turns them into a review recommendation. The public demo is fixture-backed. Quoted: fixture output must not be mistaken for a live model decision. Live Jev stays behind `POST /api/analyze`. A block recommendation is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Awlaursen/pi-lookout](https://github.com/Awlaursen/pi-lookout) (MIT, TypeScript, HEAD `667b215`) is a Pi extension that stops a running bash command. Jev is pinned to `jev-1.13.0`. It stops only when intervention needed is at least **0.7**, continuing useful is at most **0.25**, and Jev named an evidence block. Quoted: **When in doubt, it keeps running.** Commands that finish within 5 seconds never contact Jev. `lookout: false` skips the stop. Quoted: stopping the local process does not undo remote side effects. *Theirs* ten scenarios are a sanity check, not an accuracy guarantee. Distinct from [pi-jev-gate](https://github.com/fivethirty/pi-jev-gate). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[NISH1001/reflex-guard](https://github.com/NISH1001/reflex-guard) (MIT, Python, HEAD `da504a0`) is multi-label guardrails on System One models. `LayaGuard` is the shipped judge. Quoted: `VonGuard` and `JevGuard` **raise `NotImplementedError`**. Optional `threshold=0.5` flags categories; `None` only ranks. **Not TypeSafe Jev** until that class lands. **0.5** is uncalibrated. Distinct from [jagsan-cyber/reflex-gate](https://github.com/jagsan-cyber/reflex-gate) and [vuckuola619/reflex](https://github.com/vuckuola619/reflex). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[manas95826/-Semantic-Firewall-laya](https://github.com/manas95826/-Semantic-Firewall-laya) (no license file, Python, HEAD `c6cbf1f`) sits between an agent and its tools. Quoted README: **The agent is never trusted to enforce its own policy.** Laya decides; code returns `ALLOW` / `ASK` / `BLOCK`. **Not TypeSafe Jev.** `/api/evaluate` is a dry run and does not execute. Shell commands never run through a shell. *Theirs* dangerous calls allowed **0** on that laptop eval is not a rh-guard ROC. Distinct from [CeamKrier/semantic-firewall](https://github.com/CeamKrier/semantic-firewall). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jkudish/jev-agent-tools](https://github.com/jkudish/jev-agent-tools) (MIT, JavaScript, HEAD `56b6e6f`) is the transport used by [jkudish/jev-browser](https://github.com/jkudish/jev-browser) and [jkudish/jev-mcp](https://github.com/jkudish/jev-mcp). Quoted README: **fail-closed validation**. `ask()` never throws a verdict. Invalid answers return `ok: false` before usage is credited, so a malformed response cannot become a decision. Auto-selection maps `jev-latest` to `typesafe/jev-1.13` on OpenRouter and to `typesafe/jev` on Cloudflare. A transport rejection is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Flam1ngFir3ball/jev-claude-router](https://github.com/Flam1ngFir3ball/jev-claude-router) (MIT, TypeScript, HEAD `f13b0f8`) is a Claude Code plugin and a fork of [satviksinha/jev-model-router](https://github.com/satviksinha/jev-model-router). It asks Jev for a tier and an effort, then checks cost and size before switching. Quoted: **not routed** means routing failed and the turn ran on the session model. Compaction is built on [tamaratran/fast-jev-compaction](https://github.com/tamaratran/fast-jev-compaction). Routing ≠ permission. Prune ≠ deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[bigknoxy/janus](https://github.com/bigknoxy/janus) (Apache-2.0, Python, HEAD `6de9678`) is a local coding engine. Quoted: a calibrated decision model (Laya) guards the gate; a small local LLM holds the pen. **Not TypeSafe Jev.** Deterministic code owns the engine, verifier, and byte-parity rollback. *Theirs* **0 corrupt states across 45+ matrix runs** is not a rh-guard ROC. Install is `curl | sh`. Distinct from [NandhaKishorM/laya](https://github.com/NandhaKishorM/laya). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[kingd2925-beep/handoff](https://github.com/kingd2925-beep/handoff) (MIT, Python, HEAD `ebb9847`) trains a local Laya router on the operator's own chat history. **Not TypeSafe Jev.** Quoted: **Decision only, fails open.** If it is down, slow, or unsure, the answer is `strong`. It returns `cheap` or `strong` and never touches API calls. *Theirs* **3.9%** of hard tasks leak under the default confidence gate is not a rh-guard ROC. Routing ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[riefer02/laya-call-router](https://github.com/riefer02/laya-call-router) (Apache-2.0, Python, HEAD `47ce2ac`) is a dealership call-routing demo on a fine-tuned Laya checkpoint. **Not TypeSafe Jev.** Quoted: **The model chooses a label; the policy decides the next action.** Quoted: the sample is too small to establish a quality ranking. A booking rule is code. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[RubenVP2/system-one-model-gateway](https://github.com/RubenVP2/system-one-model-gateway) (MIT, Rust, HEAD `7855fde`) is a multi-tenant `POST /v1/systemone` gateway in front of Laya or `api.typesafe.ai`. Default `S1GW_AUTH` is `required`. `FAILURE_THRESHOLD` **5** opens the circuit. Fallback to TypeSafe is a configured backend, not a named lexical floor. **advertised System One gateway ≠ this sidecar.** Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[blacksinisterx/jev-router](https://github.com/blacksinisterx/jev-router) (no license file, Python, HEAD `2717a29`) scores a prompt with four Jev questions, then a tier policy in code picks the cheapest catalog model. Quoted: **No downstream model is actually called.** Mock-first, zero cost by default. Distinct from [blacksinisterx/jev-guard](https://github.com/blacksinisterx/jev-guard) (empty listing) and [reallygood83/jev-router](https://github.com/reallygood83/jev-router). Routing ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[tmbmartell/jev-mcp](https://github.com/tmbmartell/jev-mcp) (MIT, Python, HEAD `ea6c5bc`) is a stdio MCP exposing `noul`, `choice`, and `score` over OpenRouter. The README example sends a ticket to human review when choice confidence is below **0.6**. **0.6** is an example, not a calibrated deny. If the agent never calls the tools, no gate runs. Distinct from [jkudish/jev-mcp](https://github.com/jkudish/jev-mcp) and [emlama/jev-mcp](https://github.com/emlama/jev-mcp). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[agencyenterprise/jev-recipes](https://github.com/agencyenterprise/jev-recipes) (MIT, HEAD `331b6e1`) is an npm catalog. Description rewrite (*theirs*): 200+ plug-and-play recipes. Generated summary at this HEAD: **236** focused recipes. Quoted: **Confidence is a signal for your policy, not a guarantee of correctness.** `route` can return `status: 'review'` with a null route. A review outcome is a completed evaluation, not a host deny. Do not merge into `examples/`. Cousin, not this sidecar.
[bradsec/mynameisjev](https://github.com/bradsec/mynameisjev) (MIT, JavaScript, HEAD `36523dd`) is a Claude Code plugin that writes a routing note. It does not switch the session model by itself. Quoted suggestions are `/model` and a Codex handoff when that plugin is installed. A note is not a permission. Distinct from [Flam1ngFir3ball/jev-claude-router](https://github.com/Flam1ngFir3ball/jev-claude-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[dimitritholen/clouter](https://github.com/dimitritholen/clouter) (MIT, Python, HEAD `e3f1173`) is a Claude Code plugin. A hook ranks OpenRouter image, SVG, video, and speech models with Jev. Quoted: **You pick a model** from that list, or stay with Claude. Ranking ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jamesonBradfield/laya-router](https://github.com/jamesonBradfield/laya-router) (no license file, Python, HEAD `e749db9`) is a local-first tier router on port `:8090` in front of a Laya server. **Not TypeSafe Jev.** Quoted ~**8–10 ms** is theirs, not a rh-guard ROC. Routing ≠ permission. Distinct from [wangmiaozero/laya-router-skill](https://github.com/wangmiaozero/laya-router-skill). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[atmaneayoubdev/jev-ar](https://github.com/atmaneayoubdev/jev-ar) (code Apache-2.0; weights CC BY-NC 4.0; HEAD `8b99550`) is Arabic intent routing. **Not TypeSafe Jev.** An escalate flag is their fitted threshold, not a host deny. *Theirs* health-rag **38/40** next to Jev 1.13 **40/40** and Qwen3.8-27B **40/40** is not a rh-guard ROC. [Qwen3.8-27B](https://huggingface.co/Qwen/Qwen3.8-27B) is not Archer. Archer stays **promised-not-landed**. HF weights [atmaneayoub/jev-ar](https://huggingface.co/atmaneayoub/jev-ar) (sha `8b3f4de2861a`) and dataset [atmaneayoub/jev-ar-bench](https://huggingface.co/datasets/atmaneayoub/jev-ar-bench) (sha `092898d7dea8`) are the same project. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[AgentiLoop/Agent](https://github.com/AgentiLoop/Agent) (GitHub license `NOASSERTION`; Swift, HEAD `8c22d67`, 632 stars) ships `JevAdvisor.swift`. Quoted source: every helper is **fail-open** — `nil` means **no opinion**, so an unconfigured key or a TypeSafe outage can never stall the tool loop. Quoted: `ShellSafetyService` is the enforcement layer; Jev only catches what the pattern rules miss. A check failure is reported and the command is **allowed**. The GitHub description also advertises MCP support. A Jev refusal string is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[green-dalii/pi-shift-router](https://github.com/green-dalii/pi-shift-router) (MIT, TypeScript, HEAD `31a0299`, 8 stars) is per-turn model routing for Pi. Quoted README: the Jev judge is **off by default and labelled Beta**. Each tier is a fallback chain. Routing ≠ permission. Distinct from [philippdubach/pi-jev-router](https://github.com/philippdubach/pi-jev-router) and [MarcoLoDico/pi-jev-router](https://github.com/MarcoLoDico/pi-jev-router). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[WiktorB2004/llama-index-jev](https://github.com/WiktorB2004/llama-index-jev) (MIT, Python, HEAD `421afda`, 7 stars) is a LlamaIndex reranker and a tool selector. Quoted README: rerank **fails open** (original order on error); select **fails closed** unless `default_index` is set. Quoted: **Not** affiliated with TypeSafe or LlamaIndex. *Theirs* nDCG deltas are not a rh-guard ROC. A selector Choice is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[vinilana/jev-gateway-bench](https://github.com/vinilana/jev-gateway-bench) (MIT, HEAD `2465e8b`, 4 stars) scores [jev-gateway](https://github.com/vinilana/jev-gateway) with a verifier the agent **never sees**. Quoted: real Jev (`jev-latest`, moving alias) and the gateway's default thresholds. *Theirs* 120 sessions and **all 20 runs passed every hidden check** on one cell are not a rh-guard ROC. A chess pass is not a safety envelope. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[okrath/laya-ocr-guard](https://github.com/okrath/laya-ocr-guard) (no license file, Python, HEAD `687ae4c`, 3 stars) is a dual-gate CLI (`guard pre` / `guard post`) plus an LLM Gatekeeper. **Not TypeSafe Jev** and not [NandhaKishorM/laya](https://github.com/NandhaKishorM/laya). The name is the Bánh Mì sandwich pattern, not the Laya weights. An LLM approval is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[xafold/jev-router](https://github.com/xafold/jev-router) (no license file, HEAD `9e31ba4`, 2 stars) is a Claude Code proxy. Quoted: five decision trees vote, and **safety rules can raise the choice**. Security or concurrency work gets at least Opus. Raising a tier is not a deny. Distinct from [dirien/jev-router](https://github.com/dirien/jev-router) and [reallygood83/jev-router](https://github.com/reallygood83/jev-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[SamanPandey-in/jevrail](https://github.com/SamanPandey-in/jevrail) (MIT, HEAD `aaddefc`, 1 star) description rewrite (*theirs*): Jev powered pre-execution guard for terminal coding agents. Quoted README: **Status: MVP implemented.** Quoted: **policy thresholds are still starting guesses.** `fail_mode` is `ask`, `allow`, or `deny`. A starting-guess DENY is not this sidecar. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jangtrinh/design-os-generative-ui](https://github.com/jangtrinh/design-os-generative-ui) (no license file, HEAD `80b7663`, 1 star) advertises a Laya-MLX plus TypeSafe Jev cascade. Quoted config: `cascadeThreshold: 0.30`. *Theirs* **70.0%** local resolution is not a rh-guard ROC. **0.30** is uncalibrated. A cascade pick is not a tool deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[4nt0ineB/jev-from-java](https://github.com/4nt0ineB/jev-from-java) (MIT, Java, HEAD `eae28bb`) routes support triage by confidence and classifies MASSIVE intents. A printed route is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Babitdor/laya-browserUse-mcp](https://github.com/Babitdor/laya-browserUse-mcp) (size 0) is an empty listing. **advertised browser-use MCP ≠ shipped server.** Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[CSlawyer1985/dsh-jev-router](https://github.com/CSlawyer1985/dsh-jev-router) (MIT, HEAD `17541cf`) is a DSH plugin. Quoted: default is thinking-effort only; **自动模型路由** is **出厂关闭 + 硬门禁**. Quoted: **置信度不是正确率**; below `confidenceFloor` **0.5** it abstains and keeps the harness default. **0.5** is uncalibrated. Routing ≠ permission. Distinct from [codebam/dsh-jev-guardrails](https://github.com/codebam/dsh-jev-guardrails). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[DanielTea/laya-vision-stitch](https://github.com/DanielTea/laya-vision-stitch) (MIT, HEAD `37cbc31`) is screenshot-and-goal game research. **Not TypeSafe Jev.** A game policy is not a tool gate. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Diwas2055/gatelaya](https://github.com/Diwas2055/gatelaya) (Apache-2.0, Python, HEAD `e04f27c`) is a LiteLLM `CustomGuardrail` on **Laya**. **Not TypeSafe Jev.** Quoted: `GATELAYA_FAIL_OPEN` default **true** allows the request on agent failure; `false` blocks. The slim image **fail-opens** without torch. A Laya block is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[GMR714/jev-decision-lab](https://github.com/GMR714/jev-decision-lab) (no license file, HEAD `788f2e3`) is a 32-case bilingual support bench. Quoted: **an engineering diagnostic, not an estimate of real-world accuracy.** Quoted: the gate **was worse than using Jev directly**. *Theirs* **24/24** route on `jev-1.13.0` is not a rh-guard ROC. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Kamandlou/laya-local-guard](https://github.com/Kamandlou/laya-local-guard) (GitHub license `NOASSERTION`, Python, HEAD `8cce846`) is a local Laya-MLX CLI plus deterministic PII and secret checks. Quoted: **not production-ready.** **Not TypeSafe Jev.** Distinct from [NandhaKishorM/laya](https://github.com/NandhaKishorM/laya). A local score is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Kamandlou/laya-system1-snake](https://github.com/Kamandlou/laya-system1-snake) (MIT, HEAD `fd9ba74`) is a terminal Snake policy. **Not TypeSafe Jev.** A snake move is not a tool gate. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[P4A-Policies-for-Agents/Abuse-Scraping-Classifier](https://github.com/P4A-Policies-for-Agents/Abuse-Scraping-Classifier), [Declared-Purpose-Plausibility-Check](https://github.com/P4A-Policies-for-Agents/Declared-Purpose-Plausibility-Check), [EU-AI-Act-Use-Case-Risk-Tagging](https://github.com/P4A-Policies-for-Agents/EU-AI-Act-Use-Case-Risk-Tagging), [LLM-Response-Leakage-Guard](https://github.com/P4A-Policies-for-Agents/LLM-Response-Leakage-Guard), [MCP-Tool-Call-Intent-Alignment](https://github.com/P4A-Policies-for-Agents/MCP-Tool-Call-Intent-Alignment), [Risk-Tiered-Human-in-the-Loop](https://github.com/P4A-Policies-for-Agents/Risk-Tiered-Human-in-the-Loop), and [Tool-Argument-Exfiltration-Guard](https://github.com/P4A-Policies-for-Agents/Tool-Argument-Exfiltration-Guard) (each size 0) advertise Omni/Flex Gateway policies with a typed Jev judge. **advertised gateway policy ≠ shipped source.** Do not invent a 429, a 403, or a human-in-the-loop hold. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[antaeusio/nli-server](https://github.com/antaeusio/nli-server) (Apache-2.0, HEAD `e78b593`) answers yes/no policy questions with an open NLI model. Quoted: **Status: experimental.** **Not TypeSafe Jev.** An NLI entailment is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[aryanthegamedev3465-collab/tiny-decision](https://github.com/aryanthegamedev3465-collab/tiny-decision) (MIT, HEAD `77d6993`) is a local System One harness with one-click MCP export. **Not TypeSafe Jev.** An exported MCP tool is not a PreToolUse hook. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[codaaiteam/jev-agent-tools](https://github.com/codaaiteam/jev-agent-tools) (MIT, HEAD `36fdcad`) and [codaaiteam/jev-skill-router](https://github.com/codaaiteam/jev-skill-router) (MIT, HEAD `464a008`) are single-file browser tools. Quoted agent-tools: act on `if risk < 0.2`. **0.2** is uncalibrated. A browser threshold is not a host deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[dandacompany/jev-gatekeeper](https://github.com/dandacompany/jev-gatekeeper) (MIT, TypeScript, HEAD `447bc99`) is local-judge first, cloud Jev second. Quoted: sensitive at or above `t_sens` stays **LOCAL** and **Jev never called**. Ambiguous asks. A local force is code. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[danieljohnmorris/omp-jev-router](https://github.com/danieljohnmorris/omp-jev-router) (no license file, HEAD `1e472b9`) routes OMP turns. Quoted: a capability answer is a **floor**; the router may pick a higher tier, **never a lower one**. Quoted: **A routing failure never blocks the turn.** A failed classification falls back to `strong`. Routing ≠ permission. Distinct from [devjtv/jev-router](https://github.com/devjtv/jev-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[danielmeppiel/apm-laya-triage](https://github.com/danielmeppiel/apm-laya-triage) (no license file, HEAD `075cfb2`) is a read-only Laya bench on `microsoft/apm`. Quoted: **It never changes APM.** *Theirs* FP32 exact match **1 of 1,079** and a later text classifier **77.3% F1** are agreement with silver labels, not a rh-guard ROC. **Not TypeSafe Jev.** A local label file is not a close. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[dansya-arsana/jev-harness](https://github.com/dansya-arsana/jev-harness) (MIT, HEAD `fddbce7`) is a Claude Code permission gate plus skill routing. Quoted: hard rules deny or ask; **Never auto-approves.** *Theirs* **64/64** on a fresh held-out set is not a rh-guard ROC. Distinct from [AntonioCoppe/jev-harness](https://github.com/AntonioCoppe/jev-harness) and [Astro-Han/jev-harness](https://github.com/Astro-Han/jev-harness). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[dirien/jev-router](https://github.com/dirien/jev-router) (Apache-2.0, TypeScript, HEAD `d71edda`) is a pass-through proxy for Claude Code and Codex. Quoted: within a session it **only moves up**. Quoted boundary: Anthropic does not support routing Claude Code to non-Claude models through a gateway. Routing ≠ permission. Distinct from [xafold/jev-router](https://github.com/xafold/jev-router) and [reallygood83/jev-router](https://github.com/reallygood83/jev-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[ericwanderlust/jev-codex-router](https://github.com/ericwanderlust/jev-codex-router) (MIT, HEAD `ea265e9`) targets Codex GPT-6 Luna, Sol, and Astra. Quoted English: **Installing the plugin only installs its guide; it does not deploy the local router.** No key, auth failure, or no valid route falls back (usually GPT-6 Astra / medium). That fallback is **not a successful Auto decision**. Routing ≠ permission. Distinct from [0xNatoshi/jev-codex-router](https://github.com/0xNatoshi/jev-codex-router) and [nautahakk/jev-codex-router](https://github.com/nautahakk/jev-codex-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[frederico-kluser/jev-agent-skill](https://github.com/frederico-kluser/jev-agent-skill) (MIT, HEAD `2e2998f`) is a typed-decision skill. *Theirs* **8/8** at mean confidence **0.97** and ECE **0.029** is not a rh-guard ROC. A skill example is not a PreToolUse deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hrtaym1114-github/x-jev-gate](https://github.com/hrtaym1114-github/x-jev-gate) (MIT, HEAD `ae55f9a`) scores draft text. Quoted: **PASS** / **BLOCK**; **自動投稿はしません。判定のみです。** Layer A secret hits and a threshold miss exit 1. Do not invent posts. A local exit code is not a publish deny on X. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[idss-mesa/mesa-anyjev](https://github.com/idss-mesa/mesa-anyjev) (MIT, size 0, HEAD `6e4b317`) advertises AnyJev decisions over mesa-mcp. **advertised MCP ontology ≠ shipped source.** Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jerin288/layastudy](https://github.com/jerin288/layastudy) (no license file, HEAD `7810cb4`) is a Laya email-triage desk. **Not TypeSafe Jev.** A triage label is not a send. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jevlinai/jevlin-go](https://github.com/jevlinai/jevlin-go) (Apache-2.0, Go, HEAD `d7d7b49`) gives coding agents web search through the Jevlin router. Quoted: **There is no daemon, proxy or MCP server.** A search route is not a tool deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[kartikanand73/jev-decision-gateway](https://github.com/kartikanand73/jev-decision-gateway) (no license file, Python, HEAD `c0e60de`) compares Jev, GPT-6 Sol, and rules on 100 withdrawals. Quoted: a hard-rule failure rejects **and the model is never called**. Quoted: scorer error, timeout, or broken schema → manual review (**fail safe, never fail open**). Without keys, Jev and the LLM **run as mock adapters**. A mock approve is not a live payout. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[loopgridio/loopgrid-jev](https://github.com/loopgridio/loopgrid-jev) (GitHub license `NOASSERTION`, HEAD `0ec0990`) records Jev decisions. Quoted: **Jev decides. LoopGrid preserves the evidence.** A receipt is not a permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[mdad-elec/laya-v2-agent-routing](https://github.com/mdad-elec/laya-v2-agent-routing) (Apache-2.0, HEAD `f4139e4`) description rewrite (*theirs*): a 421M System-One router. **Not TypeSafe Jev.** *Theirs* held-out band accuracy **0.801** vs a 27B LLM judge **0.662** is not a rh-guard ROC. [Qwen3.8-27B](https://huggingface.co/Qwen/Qwen3.8-27B) is not this 27B judge, and neither is Archer. Archer stays **promised-not-landed**. Routing ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[rabi/pi-laya-router](https://github.com/rabi/pi-laya-router) (no license file, HEAD `0fe115c`) routes Pi through remote `laya-serve`. **Not TypeSafe Jev.** Quoted install: `pi install` **or npm/git source once published.** Routing ≠ permission. Distinct from [green-dalii/pi-shift-router](https://github.com/green-dalii/pi-shift-router). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[seberatolmez/system-one-router](https://github.com/seberatolmez/system-one-router) (no license file, Python, HEAD `94cc849`) is an OpenRouter passthrough. Quoted: **Automatic routing, streaming, and virtual `system-one/*` models are not enabled yet.** **advertised production-grade router ≠ shipped automatic routing.** Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[starhn87/jev-agent-optimizer](https://github.com/starhn87/jev-agent-optimizer) (no license file, HEAD `319e0e3`) selects model and effort for Codex and Claude Code (`jao`). A setup script is not a permission grant. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[tcruz1978-afk/jev-model-routing](https://github.com/tcruz1978-afk/jev-model-routing) (GitHub license `NOASSERTION`, HEAD `c889728`) is a Claude Code plugin. If Jev cannot pick a skill, an OpenRouter chat model picks, then Claude Code's built-in classifier. A skill attach is not a grant. Distinct from [shimo4228/jev-skill-router](https://github.com/shimo4228/jev-skill-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:roskosmos19/System-One-Router](https://huggingface.co/datasets/roskosmos19/System-One-Router) is a query-router training set (*theirs*: five reasoning levels). [hf:blazeofchi/system-one-eval](https://huggingface.co/datasets/blazeofchi/system-one-eval) quotes a small openly keyed diagnostic, **not a population benchmark**. Listings [hf:CaoHaoWei/Jev-LCT-Qwen2.5-1.5B](https://huggingface.co/CaoHaoWei/Jev-LCT-Qwen2.5-1.5B), [hf:sunziqin/millennium-jev-0.5b](https://huggingface.co/sunziqin/millennium-jev-0.5b), [hf:sunziqin/millennium-jev-1.5b](https://huggingface.co/sunziqin/millennium-jev-1.5b), [hf:Falconsai/laya-v796](https://huggingface.co/Falconsai/laya-v796), [hf:Hagwell/laya](https://huggingface.co/Hagwell/laya), [hf:ZLHAOOO/laya-mlx-zh](https://huggingface.co/ZLHAOOO/laya-mlx-zh), [hf:assafvayner/laya-dup](https://huggingface.co/assafvayner/laya-dup), [hf:baharedorsa/laya](https://huggingface.co/baharedorsa/laya), [hf:hackoffice/laya-multilingual](https://huggingface.co/hackoffice/laya-multilingual), [hf:vanty120/laya-vi](https://huggingface.co/vanty120/laya-vi), and space [hf:naufalacksaa/laya-router](https://huggingface.co/spaces/naufalacksaa/laya-router) are catalog cards. **Not TypeSafe Jev.** Not Archer. Archer stays **promised-not-landed**. Do not dump weights. [hf:atmaneayoub/jev-ar-bench](https://huggingface.co/datasets/atmaneayoub/jev-ar-bench) stays on the existing [jev-ar](https://github.com/atmaneayoubdev/jev-ar) card. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[advillalba/jev-system-one-demo](https://github.com/advillalba/jev-system-one-demo) (no license file, Python, HEAD `7200e04`) assesses shell-command risk with the `jev` package and returns `Risk(...)`. It does not wrap a host hook. A printed `risky=True` is not a deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[arthurcolle/jev-reasoning-lab](https://github.com/arthurcolle/jev-reasoning-lab) (GitHub license `NOASSERTION`, HEAD `569da0b`) is portable notebooks. Quoted: the routing example **does not execute** the selected language model. Live calls need `JEV_LAB_LIVE=1`. A notebook route is not a permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[SaiAbhijyan/Wastegate](https://github.com/SaiAbhijyan/Wastegate) (tree is `.gitignore`, `LICENSE`, and `README.md` only, HEAD `c070f3a`) describes a System-One gated coding CLI. **advertised gated CLI ≠ shipped gate.** Do not invent a cheap-model floor. Distinct from [Tech-Byte-Frontier/jevgate](https://github.com/Tech-Byte-Frontier/jevgate). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[ORiONx888/JevSentinel](https://github.com/ORiONx888/JevSentinel) (size 0, HEAD `6cf6f55`) describes crypto token-risk intelligence. **advertised early-warning engine ≠ shipped source.** Distinct from [CompleteTech-LLC-AI-Research/jev-sentinel](https://github.com/CompleteTech-LLC-AI-Research/jev-sentinel). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[3p3r/sentinel-laya](https://github.com/3p3r/sentinel-laya) (HEAD `b43ec2a`) ships `LICENSE` and a one-paragraph README: a fine-tuned Laya model for prompt injection and jailbreak, with a Jev-like interface. **advertised detector ≠ shipped weights.** **Not TypeSafe Jev.** Distinct from [ORiONx888/JevSentinel](https://github.com/ORiONx888/JevSentinel). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[pawel-debik/author-tools](https://github.com/pawel-debik/author-tools) (0BSD, Python, HEAD `dc13401`) is a Sublime Text fiction plugin. Jev rates mood, tension, and show/tell for the status bar. It is not a tool gate. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:16sulphur/laya-prompt-guard](https://huggingface.co/16sulphur/laya-prompt-guard) (Apache-2.0, sha `1c36ead5ccca`) is a Laya fine-tune card (`base_model` `convaiinnovations/laya`). **Not TypeSafe Jev.** A gut-check label is not a host deny. Distinct from [NandhaKishorM/laya](https://github.com/NandhaKishorM/laya). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:AndyInQtr/laya-decision-plugin](https://huggingface.co/AndyInQtr/laya-decision-plugin) (Apache-2.0, sha `723300125bfb`, `base_model` `convaiinnovations/laya-multilingual`) is an on-device decision-model card. **Not TypeSafe Jev.** Tool-routing tags are not a grant. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:TheUsurper09/aarogyamitra-laya-intent-router](https://huggingface.co/TheUsurper09/aarogyamitra-laya-intent-router) (sha `0925fe34b86b`) is a ModernBERT card with no license field on the API card. **Not TypeSafe Jev.** An intent label is not a clinical or tool grant. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:ai-ml-space/laya](https://huggingface.co/ai-ml-space/laya) (Apache-2.0, sha `6b481ab01da2`) republishes a Laya-family text-classification card. **advertised ai-ml-space listing ≠ the NandhaKishorM tree.** **Not TypeSafe Jev.** Distinct from [hf:Deepdive404-3/laya](https://huggingface.co/Deepdive404-3/laya). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:chaoliangUNSW/jev-style-v3](https://huggingface.co/spaces/chaoliangUNSW/jev-style-v3) (Apache-2.0 Gradio space, sha `e5d7417791dd`) is a text-classification demo tagged llm-routing and guardrails. A Gradio label is not a freeze. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[itsmostafa/system-one-connector](https://github.com/itsmostafa/system-one-connector) (MIT, Go, 303 stars, HEAD `668bb49`) is an MCP `evaluate` tool for Claude Code, Claude Desktop, Codex, Hermes, and Pi. Quoted tool reference: `min_confidence` lets a noul or choice **abstain**; `evaluate` applies it and **never sends it to the API**. Below the threshold the choice is `__uncertain__`. Quoted README: to run an open model such as Laya locally, point `TYPESAFE_BASE_URL` at your server. **advertised Laya connector ≠ bundled weights.** Install is `curl | sh`. If the agent never calls `evaluate`, no gate runs. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[neurono-ml/typed-lm](https://github.com/neurono-ml/typed-lm) (Apache-2.0, Rust, HEAD `bc77e1f`) is a single-forward-pass server whose HTTP API is **Jev-compatible**. **Not TypeSafe Jev.** Mixture-of-experts families are rejected at load. API compatibility ≠ identical judgments. A local logit is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[CogFlux/opencode-jev-guard](https://github.com/CogFlux/opencode-jev-guard) (MIT, TypeScript, HEAD `a72c910`) is an OpenCode 2 plugin. A command is safe only if the verdict is `run` at `minConfidence` (default **0.6**) and every risk is below `riskThreshold` (default **0.7**). Quoted README: if Jev cannot be reached, times out, or no key is set, the command **asks**. **It never silently runs.** Quoted: the guard **only ever adds prompts**. `autoAllow` was removed. Quoted: a project file can add categories and lower thresholds, but not switch anything off or raise a threshold — without that rule an agent could turn its own guard off. `/jev off` skips Jev; OpenCode's own permission rules still decide. Distinct from [godspede/construct-auto-classifier](https://github.com/godspede/construct-auto-classifier) and [leepokai/jev-guard](https://github.com/leepokai/jev-guard). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[itscloud0/codex-jev-native-router](https://github.com/itscloud0/codex-jev-native-router) (MIT, Python, HEAD `3bcffde`) is experimental macOS Codex Desktop and CLI routing. Quoted README: **`Jev Auto` is a picker alias, not an executor model.** Policy checks an allowlist; Jev proposes a model and effort; policy validates them. An explicit concrete model bypasses Auto. A timeout falls back to Sol. Routing ≠ permission. Distinct from [0xNatoshi/jev-codex-router](https://github.com/0xNatoshi/jev-codex-router) and [ericwanderlust/jev-codex-router](https://github.com/ericwanderlust/jev-codex-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Davidasx/pi-typesafe-approve](https://github.com/Davidasx/pi-typesafe-approve) (MIT, TypeScript, HEAD `7267888`; GitHub `size: 0` was stale) auto-approves routine Pi bash. Quoted README: **the gate never executes anything and never changes how a command runs.** Default `failMode` is **`open`**. Default danger and policy thresholds **0.3**. *Theirs* 52-command set, `p ≥ 0.3` caught ~97% of the dangerous commands — not a rh-guard ROC. Quoted: a probabilistic gate **can never weaken** a hard policy gate. `monitor` lets the command run. Distinct from [phin-tech/pi-jev-approver](https://github.com/phin-tech/pi-jev-approver) (fail-closed) and [fivethirty/pi-jev-gate](https://github.com/fivethirty/pi-jev-gate). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[rayanweragala/jev-call-router](https://github.com/rayanweragala/jev-call-router) (MIT, TypeScript, HEAD `0efb626`) routes phone calls. Quoted README: **Jev never controls the PBX directly.** `routeDirectly` skips Jev. A decision needs a known route at `minConfidence` (default **0.7**); invalid, failure, or timeout goes to `fallbackRoute`. A PBX route is not a host deny. Distinct from [riefer02/laya-call-router](https://github.com/riefer02/laya-call-router) and [SuchintK/jev-call-screener](https://github.com/SuchintK/jev-call-screener). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[turenlabs/lisa](https://github.com/turenlabs/lisa) (MIT, Python, HEAD `6210c53`) is a GitHub Action. Quoted README: **Lisa reads `.lisa.toml` from the PR's base commit, never from the PR itself.** Quoted: **Fail closed.** Unreviewed (`Coverage.failed` or `too_large`) fails the check and **must never be treated as a pass.** Default threshold **0.5**. A noul at that threshold can fail the check — soft judgment as a CI veto, not this sidecar. Distinct from [turenlabs/jast](https://github.com/turenlabs/jast). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Solizardking/jev-trader-solana](https://github.com/Solizardking/jev-trader-solana) (MIT, TypeScript, HEAD `8faeade`) is a dry-run trader. Quoted README: **Nothing is signed, nothing is submitted, no wallet is touched.** Directional ops need confidence ≥ **0.9** or they downgrade to HOLD. Missing, stale, or invalid JEV → **PAUSE** (fail closed). `MODEL=mock` is an honest stand-in. A pause is not this sidecar. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Theo-Gkisis/jevops](https://github.com/Theo-Gkisis/jevops) (no license file, Python, HEAD `05200c0`; GitHub `size: 0` was stale) advertises JEV log triage. Quoted README: **Milestone 1 in progress** — local parsing and CLI, **no AI calls yet.** `cli.py` prints clustered errors and does not call Jev. **advertised triage ≠ shipped classifier.** Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[anuragfolio/figma-jev-console-mcp](https://github.com/anuragfolio/figma-jev-console-mcp) (GitHub license `NOASSERTION`; JavaScript, HEAD `68a1af0`) is a Figma bridge. README calls it a fork of [southleft/figma-console-mcp](https://github.com/southleft/figma-console-mcp); GitHub `fork` is false. **advertised fork ≠ GitHub fork flag.** Quoted `src/jev.js`: **Jev never writes code or layers.** `JEV_MODE` can be local-first with cloud fallback. A design sync is not a PreToolUse deny. Name collision with TypeSafe Jev. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[champion19007/local-software-factory](https://github.com/champion19007/local-software-factory) (MIT, Python, HEAD `096e4e8`) is a local coding loop. **Not TypeSafe Jev** (Laya triage). Quoted README: **The MCP server is the only writer.** Quoted: **The sandbox is not a security boundary.** A sandbox copy is not this sidecar's structural deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[confident-christmasfactor2015/jev-ultrafast-mcp](https://github.com/confident-christmasfactor2015/jev-ultrafast-mcp) (MIT, Python, HEAD `8a7b25b`) is an MCP browser server. Quoted `safety.py`: the blast radius is **bounded by the server, not by prompt text.** Quoted `policy.py`: optional TypeSafe turbo is **not on the critical path**; without a key it reports unavailable and the server behaves the same. Distinct from [jiawei686/jev-ultrafast-mcp](https://github.com/jiawei686/jev-ultrafast-mcp). A domain denylist is code. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[ericmjl/pi-laya-skill-router](https://github.com/ericmjl/pi-laya-skill-router) (no license file, Python, HEAD `d3b4db3`; GitHub `size: 0` was stale) injects Pi skills from local Laya. **Not TypeSafe Jev.** `LAYA_ROUTER_THRESHOLD` default **0.3**. Quoted README: sidecar down is **fail-open**. An inject is not a grant. *Theirs* ~35 ms is not a rh-guard ROC. Distinct from [rabi/pi-laya-router](https://github.com/rabi/pi-laya-router). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[ghchinoy/dgem](https://github.com/ghchinoy/dgem) (Apache-2.0, Go, HEAD `89cf6de`) serves DiffusionGemma as a decision engine, including `POST /v1/systemone` and `dgem mcp`. **Not TypeSafe Jev.** Quoted README: thresholds and temperatures were **tuned on the evaluation items**. The entropy cascade to `gemini-3.8-flash` is theirs, not a rh-guard ROC. `gemini-3.8-flash` is not Archer. Archer stays **promised-not-landed**. A `/v1/systemone` path is not TypeSafe logits. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[lawrence3699/jev-style](https://github.com/lawrence3699/jev-style) (Apache-2.0, Python, HEAD `fc458b5`) is a local decision server plus a Claude Code `PreToolUse` guard and MCP tools. **Not TypeSafe Jev.** Quoted skill: failures give **ask**, never a silent allow; the hook **always exits 0**; hard rules can only make a verdict stricter. *Theirs* 49 labelled calls: **77.6%** agreement; **2 of 16** ask labels were allowed — not a rh-guard ROC. Quoted: **a second line of defence, not a sandbox.** Same family as [hf:chaoliangUNSW/jev-style-v3](https://huggingface.co/spaces/chaoliangUNSW/jev-style-v3) (Gradio demo, not this guard). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[majidansari786/Jev-Laya-Use-Cases-](https://github.com/majidansari786/Jev-Laya-Use-Cases-) (no license file, Python, HEAD `f1e5137`) is example scripts. **Not TypeSafe Jev.** Quoted README: **Laya only provides the classification/decision.** A refund stays with application logic. A label is not a payment. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[qualixar/jev-decision-layer](https://github.com/qualixar/jev-decision-layer) (GitHub license `NOASSERTION`; README links an MIT `LICENSE`; Python, HEAD `55bfc7f`, v1.0.0) is a Codex decision layer. Quoted README: **Execution stays with the agent.** Quoted: **Jev-only never silently falls back to Laya.** The optional prompt hook sends minimized terms, not the repository. Quoted: automatic-hook coverage **is not proved**. Not an official TypeSafe product. Routing ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[shibammitra24/jev-guard](https://github.com/shibammitra24/jev-guard) (no license file, TypeScript, HEAD `190f522`) is a browser-workflow guard. Code `decide` uses deny noul **0.85**, ask noul **0.5**, and deny risk **2.5**. `userExplicit ≥ 0.75` on a non-wildcard delete **downgrades deny to ask**. Soft userExplicit loosening a deny is not a safety proof. Distinct from [leepokai/jev-guard](https://github.com/leepokai/jev-guard), [CogFlux/opencode-jev-guard](https://github.com/CogFlux/opencode-jev-guard), and [lawrence3699/jev-style](https://github.com/lawrence3699/jev-style). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[srknkrbb/jev-ecc](https://github.com/srknkrbb/jev-ecc) (MIT, Python, HEAD `32d1be9`) is a Claude Code plugin. Quoted README: without a key, hooks stay **dry-run** and only log. Hooks stay **silent unless the project has `.claude/jev.json`**. Writes into `protected_paths` are a deterministic **deny** (Jev is not asked). Routing falls back to `--default` when confidence < **0.6**, the pick is outside `--allowed`, or Jev is unavailable. Thresholds review **0.35** / confirm **0.60** / block **0.85** are uncalibrated. Install line is `curl | sh`. A dry-run log is not a deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[twilso24/jev_router](https://github.com/twilso24/jev_router) (GitHub license `NOASSERTION`; Python, HEAD `71a921a`) routes Agent Zero models. Quoted README: **Idempotent and never-raise** — a failed sync never breaks routing. Fit floor default **0.6**. A timed-out judgment retries once, then falls back. Routing ≠ permission. Distinct from [reallygood83/jev-router](https://github.com/reallygood83/jev-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[vinaychawla-ops/jev-adk-guardrail-example](https://github.com/vinaychawla-ops/jev-adk-guardrail-example) (no license file, Python, HEAD `3e8c92c`) is a Google ADK `before_tool_callback`. Block when `P(risky) ≥ threshold` (default **0.85**). Quoted README: `fail_open` default **True** allows on a Jev outage. `python demo.py` is a mock. **0.85** is uncalibrated. Distinct from [ThiagaoBR/typesafe_agent_gates](https://github.com/ThiagaoBR/typesafe_agent_gates). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[vinaychawla-ops/jev-adk-model-router](https://github.com/vinaychawla-ops/jev-adk-model-router) (no license file, Python, HEAD `66d94e2`) is an ADK `before_model_callback`. Quoted README: the callback **never short-circuits** — it only reroutes. If Jev is unreachable or the tier is unknown, routing degrades to **always fast**, never to an error. Routing ≠ permission. Distinct from the same author's guardrail example. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Arc-Rider/arc-rider-playground](https://github.com/Arc-Rider/arc-rider-playground) (MIT, HEAD `f83818b`) description rewrite (*theirs*): live Jev and MCP examples. Smart Kanban: Jev returns a team Choice and urgency and clarification Nouls; **the app applies explicit demonstration thresholds** and places the card. Jev does not generate the title. Without a key both demos show an **offline** state. The MCP table is read-only fictional rows. A demo placement is not a deny. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Nanako0129/stingray](https://github.com/Nanako0129/stingray) (MIT, Shell, plugin **0.3.3**, HEAD `3594d63`) is a Claude Code `Stop` hook for a turn that ends half-done. Quoted README: **The hook can prompt additional work; it can never permit less.** Quoted: **Installation alone performs no actions.** Every failure path exits **0** (no key, missing `jq`, timeout, non-200, malformed scores). The only exit that blocks is **2**, and the reason goes to stderr. `STINGRAY_SHADOW=1` calls Jev and **never blocks turn completion**. `STINGRAY_TAU` default **0.5**. `STINGRAY_MAX_BLOCKS` default **3**. `STINGRAY_JEV_MODEL` default `jev-1.13.0`. A running-task count above 0 does not say the running work is what was promised; the correspondence judgement **has never been evaluated** and `STINGRAY_SHAPE3_JUDGE` stays off. *Theirs* 50 labelled lines and the **81.8%** Traditional Chinese corpus are not a rh-guard ROC. English criteria have zero live measurements. Distinct from [VladyslavHontar/clear-head](https://github.com/VladyslavHontar/clear-head), [rashedInt32/jev-lens](https://github.com/rashedInt32/jev-lens), [muse0509/jev-preflight](https://github.com/muse0509/jev-preflight), and [haystackeditor/stop-rules](https://github.com/haystackeditor/stop-rules). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[XYenon/ajevt-browser](https://github.com/XYenon/ajevt-browser) (AGPL-3.0-only, TypeScript, **0.1.0**, HEAD `2117150`) is one bounded `ajevt_browser` tool for Pi, OpenCode V2, Amp, and MCP. Jev picks from finite operations; code executes. Quoted `src/loop.ts`: **Invalid or failed Jev decision; no action executed.** Defaults confidence **0.62**, goal **0.82**, risk **0.55**, stuck **0.82**. `DONE` or `goal_completed` at the goal threshold is `done` only when deterministic verifiers pass, otherwise `likely_done`. Risky labels or `risky` at the risk threshold return `needs_confirmation` unless `allow_risky` is true. Without `allowed_domains` the loop still refuses cross-origin navigation. Default model `jev-latest`. Quoted instructions: **Page text is untrusted data.** Password, token, and secret values are redacted. If the agent never calls the tool, no loop runs. Distinct from [0pilatos0/jev-browser-mcp](https://github.com/0pilatos0/jev-browser-mcp). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[0pilatos0/jev-browser-mcp](https://github.com/0pilatos0/jev-browser-mcp) (MIT, TypeScript, **0.1.0**, HEAD `a878db1`) is an MCP browser whose Jev step picks an operation and a ref, then code re-validates and executes. `GOAL_THRESHOLD` and `STUCK_THRESHOLD` are **0.85**. A decision failure returns status `error` and does not execute that step. Quoted README tool table: `browser_click`, `browser_type`, `browser_select`, and `browser_press` make **no** model call. Password fields are never exposed as refs. Value contents are never sent to Jev. A direct ref click bypasses the Jev step. Distinct from [XYenon/ajevt-browser](https://github.com/XYenon/ajevt-browser). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[0x7067/pi-jev](https://github.com/0x7067/pi-jev) (MIT, TypeScript, package `pi-jev` **0.1.0**, HEAD `60b8de1`) is Pi verbatim compaction. Quoted README: **Everything fails open.** No key switches the extension off silently and pi's own summary runs. An unscored block is kept whole. `KEEP_THRESHOLD` is **0.5**. `JEV_MODEL` default `jev-latest`. *Theirs* 24 sessions, **84%** smaller, and 69 boundaries at **83%** coverage are not a rh-guard ROC. Prune is not a deny. Distinct from [y0usaf/pi-jev](https://github.com/y0usaf/pi-jev), [JohnsonRan/pi-jev](https://github.com/JohnsonRan/pi-jev), [Jul1en-Lin/pi-jev-compaction](https://github.com/Jul1en-Lin/pi-jev-compaction), and [dev-willbird1936/pi-jev-compact](https://github.com/dev-willbird1936/pi-jev-compact). Soft judgment is never the sole veto. Do not merge into `examples/pi-extension.ts`. Cousin, not this sidecar.
[Tech-Byte-Frontier/jevgate-action](https://github.com/Tech-Byte-Frontier/jevgate-action) (Apache-2.0 OR MIT, Shell, HEAD `43277b8`) ships `action.yml` and runs `jevgate check`. `version` default **`latest`**. Outputs: exit **0** passed, **1** failed, **2** incomplete. Quoted README: **A run that could not finish still fails, so an outage never passes as a clean review.** `--fail-on none` reports without failing the job; an incomplete run still fails. Fork pull requests do not receive secrets. The binary is [Tech-Byte-Frontier/jevgate](https://github.com/Tech-Byte-Frontier/jevgate), not this action file. Distinct from [ktsu2i/jevgate-action](https://github.com/ktsu2i/jevgate-action) (name collision; composite `action.yml` and `action.sh` on `main`, HEAD `41d5706`; **0 Releases**; quoted README **This Action has not been published yet**), [thevibeworks/jevgate](https://github.com/thevibeworks/jevgate), and [totally-tim/jev-gate](https://github.com/totally-tim/jev-gate). A cached answer is not a merge grant. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[p4u/webscout](https://github.com/p4u/webscout) (no license file, Rust, HEAD `c19f1d3`) is an MCP scout. Jev screens pages before a generative model sees them. Quoted `src/typesafe.rs`: a missing `injection` answer silently meaning **not an injection** is the failure `noul_or` exists to prevent. `injection_ceiling` default **0.55**: at or above it, the chunk never reaches the generative model. `grounding_floor` **0.60**. `TYPESAFE_MODEL` is `jev-latest`. The breaker opens after **10** consecutive failures. *Theirs* token ceilings are not a rh-guard ROC. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[saahmadnejad/cline-option-scorer](https://github.com/saahmadnejad/cline-option-scorer) (MIT, JavaScript, `@donbee/cline-option-scorer` **0.3.1**, HEAD `18352c5`) adds Choice probabilities to Cline `ask_question` options via `PreToolUse` and `PostToolUse` hooks, a plugin, and an MCP server. Quoted hook: scoring failed, **allowing as-is**. Unparsable input logs `{}` and allows. `timeoutMs` default **10000** fails open. README calls the percentages calibrated; they are Choice `probabilities`, not a rh-guard ROC. A percentage is not a deny. `@donbee/jev-mcp` keeps no audit trail. If the agent never calls the MCP tool, the hook can still score. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[jasonduncan/semantic-operators](https://github.com/jasonduncan/semantic-operators) (MIT, Python, **0.11.0**, HEAD `e3e79ad`) is a System One interface for TypeSafe Jev and Laya, from Python, the CLI, or MCP tool `ask`. Quoted README: a split or unsure model should say **"Don't know"** (`value is None`, `decided is False`). An exact tie is always undecided. A malformed response raises `ProviderError`, never a confident-looking answer. `semfilter --min` defaults to **0.8**: below it an item is **unsure**, never a match. CLI exit **0** includes don't know; **1** the provider failed; **2** the request was invalid. `jev-latest` moves; the call records the reported model. An undecided answer the flow never reads stops nothing. **0.8** is uncalibrated. If the agent never calls `ask`, no gate runs. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[imjbassi/jev-guard-ros2](https://github.com/imjbassi/jev-guard-ros2) (MIT, Python, HEAD `9f15343`) is a ROS 2 safety monitor. Quoted `decision.py`: local hard limits **never wait on the network and always win** (`max` of local and Jev). An E-stop **latches** until `reset()`. Anomaly probability above **0.85** and urgency at least **4.0** can E-stop. After **3** consecutive Jev failures, `handover_on_jev_loss` requests teleop handover. Model `jev-latest`. Timeout **0.25** s. **0.85** is uncalibrated. Jev can tighten; it cannot loosen a local E-stop. Not a coding-agent hook. Distinct from [leepokai/jev-guard](https://github.com/leepokai/jev-guard). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[ayushap18/tracegraph](https://github.com/ayushap18/tracegraph) (no license file, Python, HEAD `a4c91c6`) routes subtasks with Jev. Code blocks when `unsafe` ≥ **0.7**, clarifies when `clear` < **0.25** or confidence < **0.45**, and splits only when `multi` ≥ **0.5**. A Jev exception records the error and does not run that subtask. If every subtask fails to route, there is no merge. `claude-opus-5` is not Archer. Archer stays **promised-not-landed**. Those cuts are uncalibrated. Routing ≠ permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[geninchristophe888-spec/skillbox](https://github.com/geninchristophe888-spec/skillbox) (MIT, TypeScript, **0.1.0**, HEAD `9a015d2`) is a self-hosted skills library with scoped clients and optional Jev recommendations. Quoted `recommendations.ts`: **Never silently evaluate an alphabetic/lexical subset.** Timeout, unavailable, `not_configured`, or `catalog_limit` falls back to search with `relevance: null`. Quoted SECURITY.md: Jev sends task text and **authorized** active skill descriptions only. Removing the key prevents new model requests. A recommendation is not a new grant. Distinct from [adamjralph/skill-broker](https://github.com/adamjralph/skill-broker) and [shimo4228/jev-skill-router](https://github.com/shimo4228/jev-skill-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[guillaumeduranton-lab/lemonde-mcp](https://github.com/guillaumeduranton-lab/lemonde-mcp) (no license file, Python, HEAD `5e6c395`) is an MCP server for a personal Le Monde subscription. The cookie is sent only to `*.lemonde.fr`; redirects are checked hop by hop. `triage_news` ranks headlines as `0.7 * relevance + 0.3 * major`. A rank is not a fetch grant. `get_article` uses the cookie whether or not triage ran. If the agent never calls `triage_news`, no rank runs. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[916099/jev-cttai-plugin](https://github.com/916099/jev-cttai-plugin) (no license file, Python, HEAD `ddce4d2`) is a Codex MCP plugin. Tools `jev_decide` and `jev_list_models` call Jev through CTTAI, a third-party key host, not the TypeSafe console. `CTTAI_BASE_URL` must be HTTPS. Default model `jev-latest`. Quoted `server.py`: do not return upstream response bodies. A missing `CTTAI_API_KEY` raises. If the agent never calls `jev_decide`, no judgment runs. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[guillermojmontenegro-hub/LayaTUI-MCP](https://github.com/guillermojmontenegro-hub/LayaTUI-MCP) (no license file, Python, HEAD `af5c74a`) is a terminal UI and MCP server for PyPI `laya==0.3.20`. **Not TypeSafe Jev.** `laya_choose_action` returns a choice **without executing it**. With more than 20 actions it shortlists first. A choice is not a tool grant. Distinct from [NandhaKishorM/laya](https://github.com/NandhaKishorM/laya) and [rdutra/laya-mcp](https://github.com/rdutra/laya-mcp). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[tx-smitht/jev-focus-guard](https://github.com/tx-smitht/jev-focus-guard) (MIT, JavaScript, HEAD `449f01e`) is an unpacked Chrome extension. Local signals shortlist elements; Jev chooses `remove_ad`, `remove_distraction`, or `keep`. `confidenceThreshold` default **0.72**. Model `jev-latest`. A failed call does not hide the element. `primaryContentSignals` keeps primary content even when Jev asked to remove it (`protected_primary_content`). Quoted README: this is a DOM hider, **not a filter-list network blocker**. **0.72** is uncalibrated. HTTP **429** pauses instead of retrying. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[Razeefshaik/JevEval](https://github.com/Razeefshaik/JevEval) (no license file, Python, HEAD `87e0fca`) evaluates multi-agent routing. Quoted README: milestone 1 is a **synthetic mock benchmark**, not evidence that Jev routes well. `benchmark` defaults to mock Jev and mock candidate models. The dry run never sends a request. Answer keys are never sent as task prompts. A mock route is not a permission. Those runs are not a rh-guard ROC. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[coltonspears/JevClassifier](https://github.com/coltonspears/JevClassifier) (no license file, TypeScript, HEAD `8c7332a`) is a visual complexity demo. Simulation mode works without credentials. Code maps a score onto GPT-6 Luna, Sol, or Astra. Quoted README: the thresholds are a **transparent demonstration policy, not a benchmarked claim**. Confidence is not a guarantee of correctness. The key stays on the server. GPT-6 labels are not Archer. Archer stays **promised-not-landed**. A recommended route is not a permission. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
[hf:Emerald7664/laya-multilingual-onnx](https://huggingface.co/Emerald7664/laya-multilingual-onnx) (Apache-2.0, sha `b9c10748a5e8`) publishes ONNX weights (`model-int8.onnx`, `model-fp32.onnx`) of `convaiinnovations/laya-multilingual`. **Not TypeSafe Jev.** *Theirs* **250–300 ms**, **0.0 ms network latency**, and **100% classification accuracy** are not a rh-guard ROC. Distinct from [soyelmismo/laya-multilingual-onnx](https://huggingface.co/soyelmismo/laya-multilingual-onnx) and from the FP16 export the card names `mizchi/laya-multilingual-onnx`. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
...
ktsu2i/jevgate-action
  composite action.yml and action.sh on main; workflows ci.yml and release.yml
  HEAD 41d5706; 0 Releases; This Action has not been published yet
  name collision with Tech-Byte-Frontier/jevgate-action
...
boriscardano/herdr-jev-router
  advisory routing on stock Herdr; enforcement claims unproven
