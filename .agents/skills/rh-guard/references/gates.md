...
[peakevergreen/jevidence](https://github.com/peakevergreen/jevidence) (MIT, Python, HEAD `097f783`) turns Choice, Noul, and Score into a testable routing policy. Quoted: **Nothing in this project assigns issues, runs commands, or writes to an external system.** Output includes `applied: false`. Quoted: **All bundled judgments are constructed fixtures, not captured Jev responses.** `policy.py` teaching floors: route **0.8**, reproduction **0.85**. Quoted comment: **Teaching values, not calibrated production defaults.** Quoted: **Neither threshold is a calibrated recommendation.** API failure: status `unavailable`, retain the current queue, no invented judgment. Score does not become severity. Kev-compatible, not an official TypeSafe product. Independent educational project. Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
