...
[JimmyWesley/rlcd-gateway](https://github.com/JimmyWesley/rlcd-gateway) (Apache-2.0, Go, HEAD `1c8996d`) is an OpenAI/Anthropic-compatible gateway for Claude Code, Codex, and OpenCode. Pruning asks a System One model (`open-rlcd-local`, `open-rlcd-cloud`, or `jev` at `https://api.typesafe.ai`) a keep probability per block and **never rewrites text**. If the selector fails or exceeds `selector_timeout_ms` (**3 s**), **everything is kept**. Example decisions config: `timeout_ms` **2000**, `min_confidence` **0.5**; the `else` arm takes failures. Screenshots are synthetic demo data. A prune is not a deny. Distinct from [TexasOct/jev-gateway](https://github.com/TexasOct/jev-gateway) and [zeeshan8281/slo-router](https://github.com/zeeshan8281/slo-router). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
