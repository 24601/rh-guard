...
[Jev-Auto-Router](https://github.com/miniLV/Jev-Auto-Router) Codex skill; Jev Choice → Policy Guard. Quoted: Policy Guard **Deterministic ALLOW(plan) or DENY(reason)**. Quoted SKILL: **ALLOW (execute exactly the plan) and DENY → this unit runs in Root.** Quoted: **No evidence means no production delegation**. **advertised README ≠ shipped Jev contract** (Codex Auto Router copy). Dashboard **read-only observer**. Routing ≠ permission. Thin card. Do not merge into `examples/`. Cousin, not this sidecar.
...
miniLV/Jev-Auto-Router
  ...
  advertised README ≠ shipped Jev contract
  do not merge into examples/
