...
[davertor/jev-slop-guard](https://github.com/davertor/jev-slop-guard) (MIT, JavaScript, HEAD `1f2ee47`, badge **0.1.25**) is a Chrome MV3 extension that scores X and LinkedIn posts with one `{ slop, not_slop }` choice. Default slop threshold **70%**. Blur and the SLOP stamp default on. **Show the post** clears that for the current session. Quoted: **Scrolling is never blocked.** Pause stops new classifications; existing badges stay. Model default `jev-latest` (moving alias); `jev-1.13.0` pins a version. The key stays in `chrome.storage.local`. A blur is not a hide and not a deny. Distinct from [bornakapusta/slop-guard](https://github.com/bornakapusta/slop-guard), [SwastikGorai/unslopify](https://github.com/SwastikGorai/unslopify), and [adamnroman/slop-filter](https://github.com/adamnroman/slop-filter). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
