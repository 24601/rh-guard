...
[Sahil-coder-30/jev-langgraph-router](https://github.com/Sahil-coder-30/jev-langgraph-router) (package.json license ISC; no LICENSE file, TypeScript, package `testjev` **1.0.0**, HEAD `3d369ef`) is a LangGraph firewall plus router. The README diagram says block when **P > 0.40**. `checkSafetyWithJev` blocks at noul **>= 0.75** or severity **>= 2.0**. **advertised 0.40 diagram ≠ shipped 0.75.** Those floors are uncalibrated. No key, or a thrown call, falls through to a regex; the output-guard catch **permits** the response (`Evaluation failed; permitting safe response`). README badges **~97.4%** and **<250 ms** are not a rh-guard ROC. A route is not a grant. The success-path block is the noul alone (soft-as-veto). Soft judgment is never the sole veto. Do not merge into `examples/`. Cousin, not this sidecar.
